...
console.log(link)
		fetch(link).then(r => r.json()).then(async res => {
			if (res.response[1]) return interaction.editReply("There are multiple flights with this ICAO/IATA!")
			if (!res.response[0]) return interaction.editReply("There are no flights with this ICAO/IATA!")
			...
			interaction.editReply({
				embeds: [ new EmbedBuilder()
						...
						.setFields(
							...
							{ name: "Latitude", value: flight.lat.toString(), inline: true },
							{ name: "Longitude", value: flight.lng.toString(), inline: true },
